package com.num5268.standard.impl;
 ...
 * method에 형태는 삭제할 수 없다.
 * 
 * 이러한 규칙을 적용하여 다른 클래스와 연동할때 
 * 편히라고, 오류가 없도록 하기 위함이다
 * 
 */
public class InputServiceImplV1 implements InputService {
	...
	public Integer inputValue(String title, int start) {
		...
		while (true) {
			// intIn에 저장된 값 == inpuyValue(String title)이
			// return 해준값
		...
		while (true) {
			...
			if (intIn != null) {
				if (intIn < start || intIn > end) {
					System.out.printf("값은 %d ~ %d 까지만 입력하세요", start, end);
					continue;
				}
			}
			return intIn;
